...
class HzlResNet( nn.Module ):
    def __init__(self,
        ...
        self.conv1 = conv( 3 , num_features[0] , first_kernel_size , 2 , first_kernel_size//2 , activation_fn , use_batchnorm = use_batchnorm , bias = False  )
        if self.use_maxpool:
        ...
        for i in range( 1,len(num_blocks)):
            blocks.append( self.build_blocks(block,num_features[i-1],num_features[i] , strides[i] , num_blocks[i] ) )
        self.blocks = nn.Sequential( *blocks )
        if self.pre_activation:
            self.post_bn = nn.Sequential( nn.BatchNorm2d( num_features[-1] ) , activation_fn() )

        if self.use_maxpool:
            shape = ( input_shape[0]//4//prod(strides) , input_shape[1]//4//prod(strides) )
        else:
        ...
        if feature_layer_dim is not None:
            self.fc1 = linear( num_features[-1] * shape , feature_layer_dim , activation_fn = None , pre_activation  = False , use_batchnorm = use_batchnorm)
